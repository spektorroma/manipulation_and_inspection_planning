...
class MapEnvironment(object):
    ...
    def compute_union_of_points(self, points1, points2):
        '''
        Compute a union of two sets of inspection points.
        ...
        if len(points2) == 0:
            return points1
        final_union = np.array([p2 for p2 in points2 if not any(np.array_equal(p2, p1) for p1 in points1)])
        if len(final_union) == 0:
            return points1
        final_union = np.vstack((final_union, points1))
        return final_union
